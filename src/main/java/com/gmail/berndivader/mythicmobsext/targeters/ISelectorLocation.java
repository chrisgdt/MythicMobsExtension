...
public abstract class ISelectorLocation extends ILocationSelector {
	...
	public HashSet<AbstractLocation> applyOffsets(HashSet<AbstractLocation> targets) {
		targets.forEach(abstract_location -> {
			if(this.use_relative) {
				...
				abstract_location.add(soV.getX() + foV.getX(), this.y_offset + soV.getY() + foV.getY(),
						soV.getZ() + foV.getZ());
	...
	void sortByDistance(SkillMetadata data) {
		...
		AbstractLocation[] locations = data.getLocationTargets()
				.toArray(new AbstractLocation[data.getLocationTargets().size()]);
	...
	void nearest(SkillMetadata data) {
		AbstractLocation[] targets = data.getLocationTargets()
				.toArray(new AbstractLocation[data.getLocationTargets().size()]);
		...
		for (int i1 = 0; i1 < targets.length; i1++) {
			...
			if (nearest == null || caster_location.distance(e) < caster_location.distance(nearest))
				nearest = e;
		}
		if (nearest != null)
			data.setLocationTarget(nearest);
	}

	void shuffle(SkillMetadata data) {
		List<AbstractLocation> shuffled_locations = Arrays
				.asList(new AbstractLocation[data.getLocationTargets().size()]);
