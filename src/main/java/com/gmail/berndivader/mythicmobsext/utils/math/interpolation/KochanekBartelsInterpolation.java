...
package com.gmail.berndivader.mythicmobsext.utils.math.interpolation;
...
public class KochanekBartelsInterpolation implements Interpolation {
	...
	/**
	 * Returns the linear combination of the given coefficients with the nodes
	 * adjacent to baseIndex.
	 *
	...
	public Vector get1stDerivative(double position) {
		...
		return a.multiply(1.5 * position - 3.0 * index).add(b).multiply(2.0 * position)
				.add(a.multiply(1.5 * index).subtract(b).multiply(2.0 * index)).add(c).multiply(scaling);
	}
