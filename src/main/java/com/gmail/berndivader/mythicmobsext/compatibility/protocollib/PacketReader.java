...
public class PacketReader implements PacketListener {
	...
	public void onPacketSending(PacketEvent packet_event) {
		if (packet_event.isCancelled() || packet_event.getPlayer() instanceof TemporaryPlayer)
			return;
		...
		case 72:
			...
			player.setMetadata(Utils.meta_LASTHEALAMOUNT,
					new FixedMetadataValue(Main.getPlugin(), health_packet.getHealth() - last_health));
