...
class Reflections {
	...
	static void load_classes() throws ClassNotFoundException, NoSuchMethodException, SecurityException {
		...
		case 7:
			...
			class_RegionContainer_getRegionManager = class_RegionContainer.getMethod("get",
					com.sk89q.worldedit.world.World.class);
		...
		class_RegionManager_getApplicableRegions = class_RegionManager.getMethod("getApplicableRegions",
				class_BlockVector);
	...
	public static Object getApplicableRegions(World world, Vec3D vector) {
		...
		Object regions = null;
		try {
			region_manager = class_RegionContainer_getRegionManager.invoke(
					class_WorldGuard_getRegionContainer.invoke(worldguard),
					version > 6 ? new BukkitWorld(world) : world);
	...
	public static Object getRegion(World world, String region_name) {
		...
		try {
			region_manager = class_RegionContainer_getRegionManager.invoke(
					...
					version > 6 ? new BukkitWorld(world) : world);
			if (region_manager != null)
				region = class_RegionManager_getRegion.invoke(region_manager, region_name);
		} catch (IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
			e.printStackTrace();
		}
		return region;
