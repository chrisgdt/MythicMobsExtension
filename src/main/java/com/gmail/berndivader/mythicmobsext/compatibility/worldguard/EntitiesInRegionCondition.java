...
public class EntitiesInRegionCondition extends AbstractCustomCondition implements ILocationCondition {
			...
			for (int i1 = 0; i1 < size; i1++) {
				...
				if (type.startsWith("MYTHICMOB")) {
					...
					if (temp.length > 1) {
						...
						if (am != null && am.getType().getInternalName().toUpperCase().equals(mythicmob_type))
							return true;
					}
				} else {
					switch (type) {
					...
					default:
						if (entity.getClass().getSimpleName().toUpperCase().contains(type))
							return true;
						break;
					}
				}
	...
	public boolean check(AbstractLocation al) {
		...
		if (regions_set != null) {
			...
			List<ProtectedRegion> regions = regions_set.stream()
					.filter(r -> !region_names_exclude.contains(r.getId().toUpperCase())).collect(Collectors.toList());
			if (use_priority) {
				Optional<ProtectedRegion> maybe = regions.stream()
						.max(Comparator.comparing(ProtectedRegion::getPriority));
				if (maybe.isPresent()) {
				...
				if (!all_regions) {
					regions = regions.stream().filter(r -> region_names.contains(r.getId().toUpperCase()))
	...
	int sumEntityInRegion(World world, ProtectedRegion region) {
		...
		BoundingBox box = new BoundingBox(min_point.getX(), min_point.getY(), min_point.getZ(), max_point.getX(),
				max_point.getY(), max_point.getZ());
		return Volatile.handler.getNearbyEntities(world, box, entities).size();
	}
	...
	int sumEntityInRegions(World world, List<ProtectedRegion> regions) {
		...
		double min_x = min_point.getX(), min_y = min_point.getY(), min_z = min_point.getZ(), max_x = max_point.getX(),
				max_y = max_point.getY(), max_z = max_point.getZ();
