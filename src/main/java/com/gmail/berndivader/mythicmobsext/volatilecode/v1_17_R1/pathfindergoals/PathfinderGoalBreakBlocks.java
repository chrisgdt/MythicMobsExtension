...
public class PathfinderGoalBreakBlocks extends PathfinderGoal {
	...
	public boolean b() {
		if (this.entity.getGoalTarget() != null && this.entity.getGoalTarget().isAlive()) {
			return true;
		}
		return false;
	}
	...
	private Block getBreakableTargetBlock(EntityLiving target) {
		...
		return this.entity.getWorld().getWorld().getBlockAt((int) Math.floor(this.entity.locX() + bdx),
				(int) Math.floor(this.entity.locY()), (int) Math.floor(this.entity.locZ() + bdz));
	}
				...
				new BukkitRunnable() {
					...
					public void run() {
						...
						if (!CraftEventFactory.callEntityChangeBlockEvent(entity, position,
								entity.getWorld().getType(position).getBlock().getBlockData()).isCancelled()) {
							entity.getWorld().triggerEffect(2001, position,
									net.minecraft.world.level.block.Block.getCombinedId(entity.getWorld().getType(position)));
