...
public class PathfinderGoalInteractDoor extends PathfinderGoal {
	...
	protected BlockPosition b = BlockPosition.b; // ZERO
	...
	@Override
	public boolean a() {
		if (!this.a.A || (bl1 && !this.a.getWorld().getGameRules().getBoolean(GameRules.c))) // Entity#A : horizontalCollision, GameRules.c : MOB_GRIEDING
			return false;
	...
	private BlockDoor a(BlockPosition bp1) {
		IBlockData bd1 = this.a.getWorld().getType(bp1);
		Block b1 = bd1.getBlock();
		if ((b1 instanceof BlockDoor) && bd1.getMaterial() == Material.z) // Material.WOOD
			return (BlockDoor) b1;
		return null;
	}
}
