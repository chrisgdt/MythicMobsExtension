...
public class ModdingItem implements Cloneable {
	...
	public ModdingItem(WhereEnum where, String slot, ACTION action, Material material, String[] lore_array, String name,
			RandomDouble amount, List<Enchant> enchants, String durability, String bag_name, JsonElement json_element) {
		this.where = where;
		this.action = action;
	...
	public ItemStack applyMods(SkillMetadata data, AbstractEntity target, ItemStack item_stack) {
		...
		case SET:
			if (material.isPresent())
				item_stack.setType(material.get());
			if (amount.isPresent())
		...
		case DEL:
			...
			if (enchants.isPresent()) {
				for (Enchant enchant : enchants.get()) {
					if (item_stack.containsEnchantment(enchant.enchantment))
	...
	public ItemStack getItemStackByWhere(SkillMetadata data, AbstractEntity target, LivingEntity entity) {
		...
		switch (where) {
		case HAND:
			if ((output = entity.getEquipment().getItemInMainHand()) != null && output.getType() != Material.AIR)
				return output;
			break;
		case OFFHAND:
			if ((output = entity.getEquipment().getItemInOffHand()) != null && output.getType() != Material.AIR)
				return output;
			break;
		case HELMET:
			if ((output = entity.getEquipment().getHelmet()) != null && output.getType() != Material.AIR)
				return output;
			break;
		case CHESTPLATE:
			if ((output = entity.getEquipment().getChestplate()) != null && output.getType() != Material.AIR)
				return output;
			break;
		case LEGGINGS:
			if ((output = entity.getEquipment().getLeggings()) != null && output.getType() != Material.AIR)
				return output;
			break;
		case BOOTS:
			if ((output = entity.getEquipment().getBoots()) != null && output.getType() != Material.AIR)
				return output;
			break;
		case INVENTORY:
			if (entity instanceof Player && this.getSlot(data, target) > -1) {
				...
				if ((output = player.getInventory().getItem(slot)) != null && output.getType() != Material.AIR)
					return output;
			}
			break;
		case BACKBAG:
			if (BackBagHelper.hasBackBag(entity.getUniqueId())) {
				Inventory inventory = BackBagHelper.getInventory(entity.getUniqueId(),
						bag_name.isPresent() ? bag_name.get() : BackBagHelper.str_name);
				if (inventory != null) {
					int slot = this.getSlot(data, target);
					if (inventory.getSize() >= slot && slot > -1) {
						if ((output = inventory.getItem(slot)) != null && output.getType() != Material.AIR)
							return output;
					}
				}
			}
