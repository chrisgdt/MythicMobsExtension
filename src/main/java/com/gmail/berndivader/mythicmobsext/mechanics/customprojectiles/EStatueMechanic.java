...
public class EStatueMechanic extends SkillMechanic implements ITargetedEntitySkill, ITargetedLocationSkill {
	...
	private class ProjectileRunner implements IParentSkill, Runnable {
		...
		public void run() {
			...
			if (oldLocation.getX() != currentLocation.getX() || oldLocation.getY() != currentLocation.getY()
			...
			if (this.dur > EStatueMechanic.this.tickInterval) {
				HitBox hitBox = new HitBox(this.currentLocation, EStatueMechanic.this.hitRadius,
						EStatueMechanic.this.verticalHitRadius);
				ListIterator<Entity> lit1 = Volatile.handler.getNearbyEntities(entity, 1).listIterator();
				while (lit1.hasNext()) {
					Entity ee = lit1.next();
					if (ee == this.owner || ee == entity || ee.isDead() || !(ee instanceof LivingEntity)
							...
							|| !EStatueMechanic.this.hitNonPlayers && ee != target)
						continue;
			...
			if (!this.islocationtarget) {
				...
				NMSUtils.setLocation(this.entity, x, this.currentLocation.getY(), z,
						facedir ? MathUtils.lookAtYaw(oldLocation, currentLocation) : currentLocation.getYaw(),
						this.currentLocation.getPitch());
			} else {
				NMSUtils.setLocation(this.entity, this.currentLocation.getX(), this.currentLocation.getY(),
						this.currentLocation.getZ(), currentLocation.getYaw(), this.currentLocation.getPitch());
			}
			if (this.lifetime)
				this.count++;
			this.dur++;
		...
		public void doHit(HashSet<AbstractEntity> targets) {
			if (EStatueMechanic.this.onHitSkill.isPresent()
					&& EStatueMechanic.this.onHitSkill.get().isUsable(this.data)) {
				SkillMetadata sData = this.data.deepClone();
		...
		public void stop() {
			if (EStatueMechanic.this.onEndSkill.isPresent()
					&& EStatueMechanic.this.onEndSkill.get().isUsable(this.data)) {
				SkillMetadata sData = this.data.deepClone();
