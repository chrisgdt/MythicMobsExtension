	...
	private class ProjectileRunner implements IParentSkill, Runnable {
		...
		public void run() {
			...
			if (this.dur > MStatueMechanic.this.tickInterval) {
				HitBox hitBox = new HitBox(this.currentLocation, MStatueMechanic.this.hitRadius,
						MStatueMechanic.this.verticalHitRadius);
				...
				while (lit1.hasNext()) {
					...
					if (ee == this.owner || ee == entity || ee.isDead() || !(ee instanceof LivingEntity)
							...
							|| !MStatueMechanic.this.hitNonPlayers && ee != entity)
						continue;
		...
		public void doHit(HashSet<AbstractEntity> targets) {
			if (MStatueMechanic.this.onHitSkill.isPresent()
					&& MStatueMechanic.this.onHitSkill.get().isUsable(this.data)) {
				SkillMetadata sData = this.data.deepClone();
		...
		public void stop() {
			if (MStatueMechanic.this.onEndSkill.isPresent()
					&& MStatueMechanic.this.onEndSkill.get().isUsable(this.data)) {
				SkillMetadata sData = this.data.deepClone();
