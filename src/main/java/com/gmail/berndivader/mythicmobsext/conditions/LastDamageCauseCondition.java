...
public class LastDamageCauseCondition extends AbstractCustomCondition implements IEntityCondition {
	...
	public boolean check(AbstractEntity ae) {
		...
		if (entity.hasMetadata(Utils.meta_LASTDAMAGER))
			damager = entity.getMetadata(Utils.meta_LASTDAMAGER).get(0).asString().toUpperCase();
		if (entity.hasMetadata(Utils.meta_LASTDAMAGECAUSE))
			cause = entity.getMetadata(Utils.meta_LASTDAMAGECAUSE).get(0).asString().toUpperCase();
		if (entity.hasMetadata(Utils.meta_LASTDAMAGEAMOUNT))
			amount = entity.getMetadata(Utils.meta_LASTDAMAGEAMOUNT).get(0).asDouble();
		...
		if (dba)
			Main.logger.info("outcome:" + debug + ":damager:" + damager + ":cause:" + cause);
		return debug;
	}
}
