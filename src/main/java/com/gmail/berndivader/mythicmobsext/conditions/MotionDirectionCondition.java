...
public class MotionDirectionCondition extends AbstractCustomCondition implements IEntityCondition {
	...
	public MotionDirectionCondition(String line, MythicLineConfig mlc) {
		...
		if (!(bl1 = arr1.length == 1 && arr1[0] == "ANY")) {
			...
			for (int i1 = 0; i1 < arr1.length; i1++) {
				if ((mdt = MotionDirectionType.get(arr1[i1])) == null)
					continue;
				dirs.add(mdt);
			}
		}
	...
	public boolean check(AbstractEntity var1) {
		...
		Vec3D v3 = var1.isPlayer() ? Utils.players.getOrDefault(var1.getUniqueId(), new Vec3D(0, 0, 0))
		...
		if (v3.getX() != 0 || v3.getZ() != 0) {
			...
			MotionDirectionType mdt = MotionDirectionType
					.getMotionDirection((a - NMSUtils.getLastYawFloat(var1.getBukkitEntity()) + 630) % 360);
			if (bl2)
				var1.getBukkitEntity().setMetadata(MotionDirectionCondition.str,
						new FixedMetadataValue(Main.getPlugin(), mdt.toString()));
