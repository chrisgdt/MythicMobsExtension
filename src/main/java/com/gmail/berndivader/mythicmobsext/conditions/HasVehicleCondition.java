...
public class HasVehicleCondition extends AbstractCustomCondition implements IEntityCondition {
	...
	public HasVehicleCondition(String line, MythicLineConfig mlc) {
		super(line, mlc);
		if ((arr1 = mlc.getString(new String[] { "types", "type", "t" }, "ANY").toUpperCase().split(",")).length > 0
				&& !arr1[0].equals("ANY")) {
	...
	public boolean check(AbstractEntity var1) {
		if (var1.getBukkitEntity().getVehicle() == null)
			return false;
		if (arr1.length == 1 && arr1[0].equals("ANY"))
			return true;
		EntityType e = var1.getBukkitEntity().getVehicle().getType();
		for (String s1 : arr1) {
			if (EntityType.valueOf(s1).equals(e))
				return true;
		}
		return false;
	}
